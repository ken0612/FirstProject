...
public class ProductManageDao {
	...
	public List<ProductBean> getAllProductBeans() {
		...
		return jdbcTemplate.execute(sql, new PreparedStatementCallback<List<ProductBean>>() {
			...
			@Override
			public List<ProductBean> doInPreparedStatement(PreparedStatement ps)
	...
	public String getProductUrl(int pid) {
		String sql = "select p_imageurl from productinfo where p_id=?";
		return jdbcTemplate.queryForObject(sql, String.class, pid);
	}

	public List<ProductBean> searchProdcut(List<ProductBean> list, String search) {
		List<ProductBean> targetList = new ArrayList();
			for (ProductBean ele : list) {
				if (ele.getPname().contains(search)) {
					targetList.add(ele);
				}
			}
		return targetList;
